namespace ETorrent {
	public class Torrent : Object {
		...
		public Torrent (string path, string save_path) throws GLib.Error {
			uint8[] data;
			FileUtils.get_data (path, out data);
			metainfo = Base64.encode (data);
			this.save_path = save_path;
			created_from_path = true;
		}
		
		public Torrent.from_uri (string uri, string save_path) {
			this.path = uri;
			this.save_path = save_path;
		}
		...
		public bool created_from_path { get; private set; }
		...
		internal Value torrent_get (string parameter) {
			...
			var node = Session.receive_object (object)
				.get_object_member ("arguments")
				.get_array_member ("torrents")
				.get_object_element (0).get_member (parameter);
		...
		internal double torrent_get_double (string parameter) {
			var val = torrent_get (parameter);
		...
		public double metadata_percent_complete {
			get {
				return torrent_get_double ("metadataPercentComplete");
			}
		}
		...
		public double percent_done {
			get {
				return torrent_get_double ("percentDone");
			}
		}
		...
		public double recheck_progress {
			get {
				return torrent_get_double ("recheckProgress");
			}
		}
		...
		public double seed_ratio_limit {
			get {
				return torrent_get_double ("seedRatioLimit");
			}
			set {
		...
		public double upload_ratio {
			get {
				return torrent_get_double ("uploadRatio");
			}
		}
